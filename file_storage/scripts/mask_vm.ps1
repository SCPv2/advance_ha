# Virtual Server Masking Script for file_storage (PowerShell Version)
...
if ($Help) {
    ...
    Write-Host "  2. Comments out VM resources: vm_bastion, vm_db, vm_web, vm_app (File Storage)"
...
function Mask-VirtualServers {
    ...
    # File Storage has: vm_bastion (vm1), vm_web (vm2/vm2_2), vm_app (vm3/vm3_2), vm_db (vm4)
    $vmResources = @('vm1', 'vm2', 'vm2_2', 'vm3', 'vm3_2', 'vm4')
...
function Process-ResourceBlock {
    ...
    foreach ($line in $ResourceLines) {
        # Check for VM dependencies (vm1, vm2, vm2_2, vm3, vm3_2, vm4)
        if ($line -match "depends_on.*vm[1-4](_2)?|samsungcloudplatformv2_virtualserver_server\.(vm[1-4](_2)?)") {
...
try {
    ...
    Cyan "Summary:"
    Write-Host "  • Virtual Servers masked: vm1 (bastion), vm2/vm2_2 (web), vm3/vm3_2 (app), vm4 (db)" -ForegroundColor White
