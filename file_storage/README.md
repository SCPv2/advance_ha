...
# 기존 데이터 마이그레이션
cd ~/ceweb
sudo sudo rsync -a --dry-run media_temp/ media/  # 웹 서버2의 데이터가 파일 스토리지 데이터와 충돌하는지 사전 체크
sudo sudo rsync -a --update media_temp/ media/   # 웹 서버2의 데이터가 파일 스토리지의 데이터와 충돌할 경우 애플리케이션 서버2의 데이터가 더 최신일 경우만 덮어씀
sudo rm -r media_temp                            # 임시 폴더 삭제
```
