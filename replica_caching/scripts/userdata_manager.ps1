...
# Server types (DB removed - replaced by PostgreSQL DBaaS)
$ServerTypes = @("web", "app")
...
function Get-ServerModuleContent {
    ...
    # Use Object Storage enhanced modules for both app and web servers
    if ($ServerType -eq "app" -or $ServerType -eq "web") {
        $moduleFile = Join-Path $ModulesDir "${ServerType}_server_obj_module.sh"
    } else {
...
function New-ServerUserData {
    ...
    if ($ServerType -eq "app" -or $ServerType -eq "web") {
        Write-Success "Module loaded: ${ServerType}_server_obj_module.sh (Object Storage Enhanced)"
    } else {
